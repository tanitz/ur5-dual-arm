...
The open-box detector on a screen, live, with the numbers printed.
    ...
    scripts/ur5dual-open-box-pose --measure           # also finds small cartons
...
def run_once(frame, detector, args, log=print):
    ...
    notes = {}
    if args.measure:
        log("[+] auto size: checking the crate rim and small bright cartons")
    ...
    if found is None:
        ...
        return 1
    if args.measure:
        off = notes.get("measured_size_off")
        log("[+] measured %.0f x %.0f mm%s"
            % (found.size[0] * 1000, found.size[1] * 1000,
               "" if off is None else " (off %+.1f mm)" % (off * 1000)))
    ...
    try:
        while True:
            ...
            if key == ord(" "):
                ...
                detector.auto_size = True
                detector.reset()
                measured = True
                log("auto size enabled — checking the rim and small cartons")
            elif key == ord("s"):
