...
placement is recorded whole and every one of them constrains the answer.
    ...
    python3 calibrate/world_base.py            # measure and report
    python3 calibrate/world_base.py --apply    # and write cell.yaml
...
CALIBRATE_DIR = os.path.dirname(os.path.abspath(__file__))
OUTPUT_DIR = os.path.join(CALIBRATE_DIR, "output")
...
def _jsonable(value):
    """Turn numpy-heavy calibration data into a readable JSON value."""
...
def start_output(T_ab, report):
    """Keep the evidence for this run and a snapshot of its input config."""
    stamp = datetime.datetime.now().astimezone().strftime("%Y%m%d_%H%M%S_%f")
    session_dir = os.path.join(OUTPUT_DIR, "world_base_" + stamp)
    os.makedirs(session_dir, exist_ok=False)
    if os.path.exists(config.path):
        shutil.copy2(config.path, os.path.join(session_dir, "cell_before.yaml"))
    payload = {
        "status": "measured",
        "created": datetime.datetime.now().astimezone().isoformat(),
        "source_config": os.path.abspath(config.path),
        "T_baseA_baseB": _jsonable(T_ab),
        "report": _jsonable(report),
        "placements": [
            {"name": sample.name,
             "tcp_A_in_base_A": _jsonable(sample.M),
             "tcp_B_in_base_B": _jsonable(sample.N)}
...
def finish_output(session_dir, payload, status, include_after=False,
                  fallback_report=None):
    """Finish result.json and optionally preserve the config that was applied."""
    payload["status"] = status
    payload["finished"] = datetime.datetime.now().astimezone().isoformat()
    if fallback_report is not None:
        payload["orientation_fallback_report"] = _jsonable(fallback_report)
    if include_after and os.path.exists(config.path):
        shutil.copy2(config.path, os.path.join(session_dir, "cell_after.yaml"))
    result_path = os.path.join(session_dir, "result.json")
    with open(result_path, "w") as stream:
        json.dump(payload, stream, indent=2, ensure_ascii=False)
        stream.write("\n")
    print("calibration output saved to %s" % session_dir)
    return result_path
...
# ── writing it ────────────────────────────────────────────────────────────
session_dir, session_payload = start_output(T_ab, report)
...
if not args.apply:
    ...
    finish_output(session_dir, session_payload, "report_only",
                  fallback_report=fallback[2] if fallback is not None else None)
    cell.disconnect()
    raise SystemExit(0)
...
if report["warnings"] and not (fallback is not None and not fallback[2]["warnings"]):
    ...
    finish_output(session_dir, session_payload, "apply_refused",
                  fallback_report=fallback[2] if fallback is not None else None)
    cell.disconnect()
    raise SystemExit(2)
...
if report["warnings"]:
    ...
    finish_output(session_dir, session_payload, "applied_orientation_only",
                  include_after=True, fallback_report=fallback[2])
else:
    ...
    session_payload["report"] = _jsonable(report)
    finish_output(session_dir, session_payload,
                  "applied_full" if report["marked_calibrated"]
                  else "applied_full_not_trusted",
                  include_after=True)
